...
#include "../bundle/states/BundleActiveState.hpp"
#include "../bundle/states/BundleInstalledState.hpp"
#include "../bundle/states/BundleResolvedState.hpp"
#include "../bundle/states/BundleStartingState.hpp"
#include "../bundle/states/BundleStoppingState.hpp"
...
namespace cppmicroservices
{
    ...
    {
        SetLifecycleState(std::make_shared<BundleStartingState>());
        coreCtx->Init();
    }

    void
    FrameworkPrivate::Init()
    {
        auto l = Lock();
        US_UNUSED(l);

        auto s = GetBundleStateEnum();
        switch (s)
        {
    ...
    FrameworkPrivate::WaitForStop(std::chrono::milliseconds const& timeout)
    {
        ...
        auto l = Lock();
        // Already stopped?
        auto ws = GetBundleStateEnum();
        if (ws != Bundle::STATE_INSTALLED && ws != Bundle::STATE_RESOLVED)
        {
    ...
    void
    FrameworkPrivate::Shutdown(bool restart)
    {
        auto l = Lock();
        US_UNUSED(l);
        bool wasActive = false;
        auto shutdownState = GetBundleStateEnum();
    ...
    void
    FrameworkPrivate::Start(uint32_t)
    {
        std::vector<long> bundlesToStart;
        {
            ...
            {
                auto fwState = GetBundleStateEnum();
                switch (fwState)
                {
                    case Bundle::STATE_INSTALLED:
                    case Bundle::STATE_RESOLVED:
...

        {
            ...
            if (GetBundleStateEnum() == Bundle::STATE_ACTIVE)
            {
                return;
            }
        ...
        coreCtx->listeners.SendFrameworkEvent(
            FrameworkEvent(FrameworkEvent::Type::FRAMEWORK_STARTED, MakeBundle(shared_from_this()), std::string()));
    }

    void
    FrameworkPrivate::Stop(uint32_t)
    {
        Shutdown(false);
    }
    ...
    std::string
    FrameworkPrivate::GetLocation() const
    {
    ...
    AnyMap const&
    FrameworkPrivate::GetHeaders() const
    {
        return headers;
    }

    void
    FrameworkPrivate::Shutdown0(bool restart, bool wasActive)
    {
        try
        {
            {
        ...
        catch (...)
        {
            auto l = Lock();
            US_UNUSED(l);
            SystemShuttingdownDone_unlocked(FrameworkEventInternal { true,
                                                                     FrameworkEvent::Type::FRAMEWORK_ERROR,
                                                                     std::string(),
                                                                     std::current_exception() });
        }
    ...
    void
    FrameworkPrivate::ShutdownDone_unlocked(bool restart)
    {
        auto t = restart ? FrameworkEvent::FRAMEWORK_STOPPED_UPDATE : FrameworkEvent::FRAMEWORK_STOPPED;
        SystemShuttingdownDone_unlocked(FrameworkEventInternal { true, t, std::string(), std::exception_ptr() });
    }
    ...
    FrameworkPrivate::StopAllBundles()
    {
        ...
        for (auto iter = activeBundles.rbegin(); iter != activeBundles.rend(); ++iter)
        {
            ...
            try
            {
                auto bs = b->GetBundleStateEnum();
                if (bs == Bundle::STATE_ACTIVE || bs == Bundle::STATE_STARTING)
                {
    ...
    {
        if (GetBundleStateEnum() != Bundle::STATE_INSTALLED)
        {
