...
void ChangeNamespaceImpl::copy_path(const fs::path& p)
{
   ...
   if(m_namespace_name.size() && (is_source_file(p) || is_test_config_file(p)))
   {
      ...
      std::vector<std::string> patterns = {
        "(namespace\\s+)cppmicroservices(_\\w+)?",
        "(<)cppmicroservices((?:_\\w+)?\\s*(?:::))",
        "(namespace\\s+\\w+\\s*=\\s*(?:::\\s*)?)cppmicroservices(_\\w+)?",
        "(^\\s*#\\s*define\\s+\\w+\\s+)cppmicroservices((?:_\\w+)?\\s*)$",
        "(\\(\\s*)cppmicroservices(\\s*\\))",
        "()cppmicroservices((?:_\\w+)?(?:::))"
      };

      std::string replacement = "$1" + m_namespace_name + "$2";

      for (size_t i = 0; i < patterns.size(); ++i) {
          std::regex re(patterns[i]);
          std::smatch match;
      ...
      if(m_namespace_alias)
      {
         ...
         std::regex_replace(std::back_inserter(v2), v1.begin(), v1.end(), namespace_alias,"$1 $3 {} $1 cppmicroservices   = $3; $1$2$3$4");
         std::swap(v1, v2);
         v2.clear();
      }
      ...
      std::string pattern = "(\")cppmicroservices(::([^\"]*)\")";
...
void ChangeNamespaceImpl::create_path(const fs::path& p)
{
   if(!fs::exists(m_dest_path / p))
   {
