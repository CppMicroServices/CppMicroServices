...
static void AnyMapToJsoncons(benchmark::State& state)
{
  auto framework = FrameworkFactory().NewFramework();
  framework.Start();
  auto context = framework.GetBundleContext();
...
static void ParseManifestToString(benchmark::State& state) {
  std::string path(US_FRAMEWORK_SOURCE_DIR);
  path += "/test/bundles/dummyService/resources/manifest.json";
...
static void ParseManifestToAnyMap(benchmark::State& state) {
  std::string path(US_FRAMEWORK_SOURCE_DIR);
  path += "/test/bundles/dummyService/resources/manifest.json";
...
static void ParseManifestToJsoncons(benchmark::State& state) {
  std::string path(US_FRAMEWORK_SOURCE_DIR);
  path += "/test/bundles/dummyService/resources/manifest.json";
  for (auto _ : state) {
    std::ifstream is(path);
