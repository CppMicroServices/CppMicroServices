...
public:
  ...
  {
    framework = std::make_shared<cppmicroservices::Framework>(
...
(benchmark::State& state)
{
  ...
  cppmicroservices::ServiceTracker<Foo> fooTracker(
    fc, serviceReg.GetReference<Foo>());
  for (auto _ : state) {
...
(benchmark::State& state)
{
  ...
  for (int64_t i = 0; i < maxServiceTrackers; ++i) {
    auto fooTracker =
      std::make_unique<cppmicroservices::ServiceTracker<Foo>>(fc);
    fooTracker->Open();
