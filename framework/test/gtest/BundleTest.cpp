...
public:
  ...
  void frame020b(const std::string& tempPath)
  {
    ...
    ASSERT_EQ(bc.GetProperty(Constants::FRAMEWORK_STORAGE).ToString(),
              tempPath);

    // launching properties should be accessible through any bundle
    //Test for valid base storage path
    ASSERT_EQ(buA.GetBundleContext()
    ...
    ASSERT_EQ(buA.GetBundleContext().GetDataFile("bla"),
              (baseStoragePath + "bla"));
  ...
  void frame037a()
  {
    ...
    //Test for valid data path
    ASSERT_EQ(buExec.GetBundleContext().GetDataFile("").substr(
                0, baseStoragePath.size()),
              baseStoragePath);
    //Test for valid data file path
    ASSERT_EQ(buExec.GetBundleContext().GetDataFile("bla").substr(
                0, baseStoragePath.size()),
              baseStoragePath);

    //Test for non-empty framework uuid property
    ASSERT_FALSE(
      buExec.GetBundleContext().GetProperty(Constants::FRAMEWORK_UUID).Empty());
  ...
  void frame045a()
  {
    ...
    EXPECT_THROW(bc.AddServiceListener(
...
{
  cppmicroservices::testing::TempDir frameworkStorage =
    cppmicroservices::testing::MakeUniqueTempDirectory();
...
{
  context.InstallBundles(BIN_PATH + util::DIR_SEP + "usFrameworkTests" +
                         US_EXE_EXT);
...
{
#ifdef US_BUILD_SHARED_LIBS
  context.InstallBundles(LIB_PATH + util::DIR_SEP + US_LIB_PREFIX +
                         "TestBundleExt" + US_LIB_POSTFIX + ".cppms");
...
TEST_F(BundleTest, TestBundleManifestFailures)
{
  ...
  std::vector<std::string> validBundleNames(
    { framework.GetSymbolicName(), "main" });
  ...
  EXPECT_THROW(InstallLib(bc, "TestBundleWithInvalidVersionType"),
               std::runtime_error);
  ...
  EXPECT_THROW(InstallLib(bc, "TestBundleWithInvalidVersion"),
               std::runtime_error);
  //Test that an invalid bundle.version type results in not installing the bundle.
  ASSERT_EQ(2, bc.GetBundles().size());
  ASSERT_TRUE(validateBundleNames(validBundleNames));
  ...
  EXPECT_THROW(InstallLib(bc, "TestBundleWithoutBundleName"),
               std::runtime_error);
  ...
  EXPECT_THROW(InstallLib(bc, "TestBundleWithEmptyBundleName"),
               std::runtime_error);
...
TEST_F(BundleTest, TestBundleActivatorFailures)
{
  ...
  auto token = context.AddFrameworkListener(std::bind(
  ...
  ASSERT_TRUE(listener.CheckEvents(std::vector<FrameworkEvent>{ FrameworkEvent{
