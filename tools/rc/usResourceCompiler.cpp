...
const option::Descriptor usage[] =
{
  ...
  {UNKNOWN,      0, "" ,  ""   , Custom_Arg::None, "\nAppend a bundle with resources\n"
    "  " US_PROG_NAME " -v -b mybundle -a bundle.dylib -r manifest.json -m filetomerge.zip\n" },
  {UNKNOWN,      0, "" ,  ""   , Custom_Arg::None, "\nAppend a bundle binary with existing zip file\n"
    "  " US_PROG_NAME " -o mybundle.dll -m filetomerge.zip\n" },
  {0,0,0,0,0,0}
};
...
int main(int argc, char** argv)
{
  ...
  if (parse.error())
  {
    std::cerr << "Parsing command line arguments failed. Check usage. " << std::endl;
    return_code = EXIT_FAILURE;
  }
  
  option::Option* appendbinaryopt = options[APPENDBINARY];
  if (appendbinaryopt && appendbinaryopt->count() > 1 ) {
    std::cerr << "--append-binary appears multiple times in the arguments. Check usage. " << std::endl;
    return_code = EXIT_FAILURE;
  }
  
  option::Option* outfileopt = options[OUTFILE];
  if (outfileopt && outfileopt->count() > 1 ) {
    std::cerr << "(--out-file | -o) appears multiple times in the arguments. Check usage. " << std::endl;
    return_code = EXIT_FAILURE;
  }
  ...
  if (!appendbinaryopt && !outfileopt) {
    std::clog << "WARNING: Neither a binary file nor an output zip file is specified. Output will be saved to a temporary file." << std::endl;
  }
  ...
  if (!options[VERBOSE])
  {
    // if not in verbose mode, set the clog stream to not output anything
