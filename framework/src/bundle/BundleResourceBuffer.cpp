...
namespace cppmicroservices::detail
{
    ...
    public:
        BundleResourceBufferPrivate(std::unique_ptr<void, void (*)(void*)> data,
            ...
            , current(begin)
            , mode(mode)
            , uncompressedData(static_cast<unsigned char*>(data.release()), data.get_deleter())
...
#endif
        auto peekIt = std::next(d->current, backOffset);
        if (d->current == d->begin || (ch != traits_type::eof() && ch != *peekIt))
        {
            return traits_type::eof();
        }

        std::advance(d->current, backOffset);
        return traits_type::to_int_type(*d->current);
    }
