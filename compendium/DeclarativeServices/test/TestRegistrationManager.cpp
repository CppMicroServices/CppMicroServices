    ...
    namespace scrimpl
    {
        ...
        class MockServiceFactory : public cppmicroservices::ServiceFactory
        {
          public:
            ...
            GetService(cppmicroservices::Bundle const&, cppmicroservices::ServiceRegistrationBase const&)
            {
                callcount++;
            ...
            virtual void
            UngetService(cppmicroservices::Bundle const&,
        ...
        TEST_F(RegistrationManagerTest, VerifyConstructor)
        {
            ...
            EXPECT_THROW({ RegistrationManager rm(BundleContext(), services, SCOPE_SINGLETON, fakeLogger); },
                         std::invalid_argument)
                << "Invalid bundle context must result in a throw";
            // invalid service interfaces vector
            EXPECT_THROW({ RegistrationManager rm(bc, {}, SCOPE_SINGLETON, fakeLogger); }, std::invalid_argument)
                << "Empty services param must result in a throw";
            // invalid service scope
            EXPECT_THROW({ RegistrationManager rm(bc, services, "fuzzy", fakeLogger); }, std::invalid_argument)
                << "Invalid service scope must result in a throw";
            // invalid logger
            EXPECT_THROW({ RegistrationManager rm(bc, services, SCOPE_SINGLETON, nullptr); }, std::invalid_argument)
                << "Invalid logger must result in a throw";

            EXPECT_NO_THROW({ RegistrationManager rm(bc, services, SCOPE_SINGLETON, fakeLogger); })
        ...
        {
            auto fakeLogger = std::make_shared<FakeLogger>();
            std::vector<std::string> services { us_service_interface_iid<dummy::Reference1>(),
            ...
            EXPECT_TRUE(rm.RegisterService(std::dynamic_pointer_cast<ServiceFactory>(mockServiceFactory), props))
            ...
            InterfaceMapPtr iMap
                = MakeInterfaceMap<dummy::Reference2, dummy::Reference1>(std::make_shared<MockService>());
            ...
            EXPECT_CALL(*mockServiceFactory, UngetService(testing::_, testing::_, testing::_)).Times(1);
