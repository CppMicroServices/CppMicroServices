 ...
 *   from which bundles should be auto-loaded.
 *
...
public:
   ...
   *
   * When a bundle is loaded and this string has been added as a path
   * to the list of auto-load paths the CppMicroServices library will
   * auto-load all bundles from the currently being loaded bundle's
   * auto-load directory.
   *
...
private:
  ...
  BundleSettings(const BundleSettings&);
  BundleSettings& operator=(const BundleSettings&);
};
