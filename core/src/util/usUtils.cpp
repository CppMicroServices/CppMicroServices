    ...
    while ((ent = readdir(dir)) != NULL)
    {
      ...
      for (mz_uint fileIndex = 0; fileIndex < numFiles; ++fileIndex)
      {
        ...
        if(std::string::npos != pos)
        {
          ...
          {
            loadedBundles.push_back(installedBundle->GetName());
          }
        }
      }
...
std::vector<std::string> AutoLoadBundles(const BundleInfo& bundleInfo, CoreBundleContext* coreCtx)
{
       ...
       i != autoLoadPaths.end(); ++i)
  {
    ...
    {
      // Install all bundles from a directory located relative to this bundles location
       ...
       i != autoLoadPaths.end(); ++i)
  {
    ...
    loadedBundles.insert(loadedBundles.end(), paths.begin(), paths.end());
  }
  return loadedBundles;
}
