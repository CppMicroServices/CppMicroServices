...
void TestServiceFactoryBundleScope(BundleContext context)
{
  ...
  US_TEST_CONDITION(registeredRefs[5].GetProperty(Constants::SERVICE_SCOPE).ToString() == Constants::SCOPE_BUNDLE, "Sixth service is prototype scope");
  US_TEST_CONDITION(registeredRefs[6].GetProperty(Constants::SERVICE_SCOPE).ToString() == Constants::SCOPE_PROTOTYPE, "Seventh service is prototype scope");
  ...
  US_TEST_CONDITION_REQUIRED(service && service->size() >= 1, "Returned at least 1 service object");
...
void TestServiceFactoryBundleScopeErrorConditions()
{
  ...
  struct : detail::MultiThreaded<>,  std::vector<FrameworkEvent> {} fwEvents;
  ...
  US_TEST_CONDITION_REQUIRED(nullptr == context.GetService(svcRecursiveGetServiceRefs[0]), "Test that the service object returned is a nullptr");
  US_TEST_CONDITION_REQUIRED(2 == fwEvents.size(), "Test that one FrameworkEvent was sent");
  US_TEST_CONDITION_REQUIRED(fwEvents[0].GetType() == FrameworkEvent::FRAMEWORK_ERROR, "Test for correct framwork event type");
  US_TEST_CONDITION_REQUIRED(fwEvents[0].GetBundle() == context.GetBundle(), "Test for correct framwork event bundle");
  ...
  {
    US_TEST_CONDITION_REQUIRED(exc.GetType() == ServiceException::FACTORY_RECURSION, "Test for correct service exception type (recursion)");
  });
#endif
  ...
  fwEvents.clear();
  US_TEST_CONDITION_REQUIRED(nullptr != GetBundleContext().GetService(svcRecursiveGetServiceRefs[0]), "Test that the service object returned is not a nullptr");
  US_TEST_CONDITION_REQUIRED(fwEvents.empty(), "Test that no FrameworkEvent was sent");
