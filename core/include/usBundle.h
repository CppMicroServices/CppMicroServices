 ...
 * An <code>STOPPED</code> bundle is a
...
public:
  ...
  /**
   * Returns the property key with a value of \c bundle.autoloaded_bundles for
   * looking up this bundle's auto-load bundles.
   * The property value is of type \c std::vector<std::string> and contains
   * the file system locations for the auto-loaded bundles triggered by this
   * bundle.
   *
   * @return The auto-loaded bundles property key.
   */
  static const std::string& PROP_AUTOLOADED_BUNDLES();
   ...
   *         <code>BundleContext</code>.
   */
  virtual BundleContext* GetBundleContext() const;
   ...
   * <li>Does not change when a bundle is restarted.
   * </ul>
   *
   * <p>
   ...
   * while this bundle is in the <code>UNLOADED</code> state.
   *
   ...
   * this bundle is in the <code>UNLOADED</code> state.
   *
   ...
   */
  virtual BundleVersion GetVersion() const;
   ...
   * be found in this bundle or the bundle's state is \c UNLOADED, an invalid
   ...
   */
  virtual std::vector<BundleResource> FindResources(const std::string& path, const std::string& filePattern, bool recurse) const;
 ...
 * \ingroup MicroServices
 */
US_Core_EXPORT std::ostream& operator<<(std::ostream& os, const US_PREPEND_NAMESPACE(Bundle)& bundle);
/**
 * \ingroup MicroServices
 */
US_Core_EXPORT std::ostream& operator<<(std::ostream& os, US_PREPEND_NAMESPACE(Bundle) const * bundle);
