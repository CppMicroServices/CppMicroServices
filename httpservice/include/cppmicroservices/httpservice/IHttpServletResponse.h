...
#ifndef CPPMICROSERVICES_IHTTPSERVLETRESPONSE_H
#define CPPMICROSERVICES_IHTTPSERVLETRESPONSE_H
...
class US_HttpService_EXPORT IHttpServletResponse
{
public:
  ...
  virtual ~IHttpServletResponse() = default;
  ...
  virtual void SetBufferSize(std::size_t size) = 0;

  virtual void SetCharacterEncoding(const std::string& charset) = 0;

  virtual void SetContentLength(std::size_t size) = 0;
  ...
  virtual void SetDateHeader(const std::string& name, long long date) = 0;
  ...
  virtual void SendError(int statusCode,
                         const std::string& msg = std::string()) = 0;
...
#endif // CPPMICROSERVICES_IHTTPSERVLETRESPONSE_H
