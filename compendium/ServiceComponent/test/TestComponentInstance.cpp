...
namespace {
  ...
  class TestServiceImpl1 final : public TestServiceInterface1, public TestServiceInterface2 {
  public:
    TestServiceImpl1()
    : foo(nullptr)
    , bar(nullptr)
  ...
  /**
   * This test point is used to verify the service component object is
   * instantiated when injection is opted-out
   */
  TEST(ComponentInstance, VerifyWithoutDependencyInjection)
  {
    std::vector<std::shared_ptr<Binder<TestServiceImpl1>>> binders;
    binders.push_back(std::make_shared<StaticBinder<TestServiceImpl1, ServiceDependency1>>("foo"));
    binders.push_back(std::make_shared<StaticBinder<TestServiceImpl1, ServiceDependency2>>("bar"));
    ComponentInstanceImpl<TestServiceImpl1, std::tuple<>, std::false_type, ServiceDependency1, ServiceDependency2> compInstance(binders);
    auto iMap = compInstance.GetInterfaceMap();
    ASSERT_FALSE(iMap);
  ...
  /**
   * This test point is used to verify the injection works properly
   * for a Service Component with static dependencies
   */
  TEST(ComponentInstance, VerifyWithStaticDependencies)
  {
    ...
    ComponentInstanceImpl<TestServiceImpl1, std::tuple<>, std::true_type, ServiceDependency1, ServiceDependency2> compInstance(binders);
    auto iMap = compInstance.GetInterfaceMap();
    ASSERT_FALSE(iMap);
    ...
    EXPECT_THROW(compInstance.InvokeBindMethod("foo", fc.GetServiceReference<ServiceDependency1>()), std::runtime_error);
    EXPECT_THROW(compInstance.InvokeBindMethod("bar", fc.GetServiceReference<ServiceDependency2>()), std::runtime_error);
    EXPECT_THROW(compInstance.InvokeUnbindMethod("foo", fc.GetServiceReference<ServiceDependency1>()), std::runtime_error);
    EXPECT_THROW(compInstance.InvokeUnbindMethod("bar", fc.GetServiceReference<ServiceDependency2>()), std::runtime_error);
  ...
  /**
   * This test point is used to verify the injection works properly
   * for a Service Component with dynamic dependencies
   */
  TEST(ComponentInstance, VerifyWithDynamicDependencies)
  {
    auto f = cppmicroservices::FrameworkFactory().NewFramework();
    f.Start();
    auto fc = f.GetBundleContext();
