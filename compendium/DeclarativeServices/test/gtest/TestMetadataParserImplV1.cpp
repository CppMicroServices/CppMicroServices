...
namespace
{
        ...
        GetTestManifest(std::string const& manifest_name)
        {
            ...
            try
            {
                auto const& testMetadata
                    = headers.AtCompoundKey("test_metadata." + manifest_name + "." + SERVICE_COMPONENT);
    ...
    {
      protected:
        MetadataParserImplV1Test() : logger(std::make_shared<FakeLogger>()) {}
    ...
    TEST_F(MetadataParserImplV1Test, ParseValidManifest)
    {
        ...
        auto components
            = metadataparser->ParseAndGetComponentsMetadata(ManifestHelper::GetTestManifest("manifest_json"));
    ...
    TEST_F(MetadataParserImplV1Test, ParseDynManifest)
    {
        auto metadataparser = MetadataParserFactory::Create(1, GetLogger());
        auto components
            = metadataparser->ParseAndGetComponentsMetadata(ManifestHelper::GetTestManifest("manifest_dyn"));
        auto const component = components[0];
        ASSERT_EQ(component->activateMethodName, std::string("Activate"));
        ASSERT_EQ(component->deactivateMethodName, std::string("Deactivate"));
        ASSERT_EQ(component->modifiedMethodName, std::string("Modified"));
        ...
        ASSERT_EQ(cppmicroservices::any_cast<std::string>(component->properties["foo"]), std::string("bar"));
        ASSERT_EQ(cppmicroservices::any_cast<int>(component->properties["num_comps"]), 42);
    }
    ...
    TEST_F(MetadataParserImplV1Test, ParseMultComps)
    {
        auto metadataparser = MetadataParserFactory::Create(1, GetLogger());
        auto components
            = metadataparser->ParseAndGetComponentsMetadata(ManifestHelper::GetTestManifest("manifest_mult_comp"));
    ...
    {
        MetadataInvalidManifestState(std::string manifestName, std::string _errorOutput, bool _isPartial = false)
    ...
    class InvalidMetadataTest : public ::testing::TestWithParam<MetadataInvalidManifestState>
    {
      protected:
        std::shared_ptr<FakeLogger> logger;

        InvalidMetadataTest() : logger(std::make_shared<FakeLogger>()) {}
    ...
    TEST_P(InvalidMetadataTest, TestMetadataFailureModes)
    {
        ...
        try
        {
            AnyMap const& scr = ManifestHelper::GetTestManifest(ims.manifestName);
    ...
    INSTANTIATE_TEST_SUITE_P(
        FailureModes,
        InvalidMetadataTest,
        testing::Values(MetadataInvalidManifestState("manifest_illegal_ver", "Unsupported manifest file version '0'"),
                        MetadataInvalidManifestState("manifest_missing_ver", "Missing key 'version' in the manifest."),
                        ...
                        MetadataInvalidManifestState("manifest_illegal_ver3",
                                                     "cppmicroservices::BadAnyCastException",
                                                     /*isPartial=*/true),
                        MetadataInvalidManifestState("manifest_illegal_comp",
                                                     "Value for the name 'components' cannot be empty.",
                                                     /*isPartial=*/true),
                        MetadataInvalidManifestState("manifest_no_comp", "Missing key 'components' in the manifest.")));

    // Test failure modes where the exceptions are eaten by MetadataParser
    // and logged through the logger
    class InvalidMetadataTestThroughLogger : public ::testing::TestWithParam<MetadataInvalidManifestState>
    {
      public:
        static void
        SetUpTestCase()
    ...
    class CustomLogger : public FakeLogger
    {
      public:
        ...
        void
        Log(cppmicroservices::logservice::SeverityLevel, std::string const& message) override
        {
            msg = message;
        }
    };
    ...
    TEST_P(InvalidMetadataTestThroughLogger, TestMetadataFailureModes)
    {
        ...
        AnyMap const& scr = ManifestHelper::GetTestManifest(imsl.manifestName);
        auto version = ObjectValidator(scr, "version").GetValue<int>();
    ...
    INSTANTIATE_TEST_SUITE_P(
        ...
        testing::Values(
            MetadataInvalidManifestState("manifest_badcomp_index_1", "Could not load the component with index: 1"),
            MetadataInvalidManifestState("manifest_illegal_immediate",
                                         "Invalid value specified for the name 'immediate'. Could not load the "
                                         "component with index: 0"),
            MetadataInvalidManifestState("manifest_no_impl_class",
            ...
            MetadataInvalidManifestState("manifest_illegal_service", "cppmicroservices::BadAnyCastException"),
            MetadataInvalidManifestState("manifest_no_interfaces",
                                         "Missing key 'interfaces' in the manifest. "
                                         "Could not load the component with index: 0"),
            MetadataInvalidManifestState("manifest_empty_interfaces_arr",
                                         "Value for the name 'interfaces' cannot be empty."),
            MetadataInvalidManifestState("manifest_empty_impl_class",
                                         "Value for the name 'implementation-class' cannot be empty."),
            MetadataInvalidManifestState("manifest_illegal_ref_arr", "Value cannot be empty."),
            MetadataInvalidManifestState("manifest_empty_ref", "Value for the name 'references' cannot be empty."),
            MetadataInvalidManifestState("manifest_empty_ref_map", "Value cannot be empty."),
            MetadataInvalidManifestState("manifest_empty_ref_name", "Value for the name 'name' cannot be empty."),
            MetadataInvalidManifestState("manifest_illegal_ref_name", "cppmicroservices::BadAnyCastException:"),
            MetadataInvalidManifestState("manifest_empty_ref_interface",
                                         "Value for the name 'interface' cannot be empty."),
            MetadataInvalidManifestState("manifest_illegal_ref_interface",
                                         "Unexpected type for the name 'interface'. Exception: "
                                         "cppmicroservices::BadAnyCastException: "),
            MetadataInvalidManifestState("manifest_illegal_scope",
                                         "Invalid value 'global'. The valid choices are : [bundle, prototype, "
                                         "singleton]. Could not load the component with index: 0"),
            MetadataInvalidManifestState("manifest_illegal_ref",
                                         "Unexpected type for the name 'references'. Exception: "
                                         "cppmicroservices::BadAnyCastException"),
            MetadataInvalidManifestState("manifest_empty_interfaces_string", "Value cannot be empty.")));
