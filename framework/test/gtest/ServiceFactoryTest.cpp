...
TEST_F(ServiceFactoryTest, TestGetServiceReturnsIncompleteMap)
{
  ...
  EXPECT_CALL(*sf, UngetService(::testing::_, ::testing::_, ::testing::_))
    .Times(0);
...
TEST_F(ServiceFactoryTest, TestServiceFactoryPrototypeScope)
{
  ...
  ASSERT_TRUE(sr1);
  ASSERT_EQ(sr1.GetProperty(Constants::SERVICE_SCOPE).ToString(),
  ...
  ASSERT_TRUE(sr2);
  ASSERT_EQ(sr2.GetProperty(Constants::SERVICE_SCOPE).ToString(),
            Constants::SCOPE_PROTOTYPE);
...
TEST_F(ServiceFactoryTest, TestServiceFactoryBundleScope)
{
  ...
  ASSERT_EQ(registeredRefs[0].GetProperty(Constants::SERVICE_SCOPE).ToString(),
            Constants::SCOPE_BUNDLE);
  //Test that Second service is bundle scope
  ASSERT_EQ(registeredRefs[1].GetProperty(Constants::SERVICE_SCOPE).ToString(),
            Constants::SCOPE_BUNDLE);
  //Test that Third service is bundle scope
  ASSERT_EQ(registeredRefs[2].GetProperty(Constants::SERVICE_SCOPE).ToString(),
            Constants::SCOPE_BUNDLE);
  //Test that Fourth service is bundle scope
  ASSERT_EQ(registeredRefs[3].GetProperty(Constants::SERVICE_SCOPE).ToString(),
            Constants::SCOPE_BUNDLE);
  //Test that Fifth service is bundle scope
  ASSERT_EQ(registeredRefs[4].GetProperty(Constants::SERVICE_SCOPE).ToString(),
            Constants::SCOPE_BUNDLE);
  //Test that Sixth service is prototype scope
  ASSERT_EQ(registeredRefs[5].GetProperty(Constants::SERVICE_SCOPE).ToString(),
            Constants::SCOPE_BUNDLE);
  //Test that Seventh service is prototype scope
  ASSERT_EQ(registeredRefs[6].GetProperty(Constants::SERVICE_SCOPE).ToString(),
            Constants::SCOPE_PROTOTYPE);
...
TEST_F(ServiceFactoryTest, TestConcurrentServiceFactory)
{
  ...
  auto bundle = cppmicroservices::testing::InstallLib(
    framework2.GetBundleContext(), "TestBundleH");
  bundle.Start();

  std::vector<std::thread> worker_threads;
  for (size_t i = 0; i < 100; ++i) {
    worker_threads.push_back(std::thread([framework2]() {
...
TEST_F(ServiceFactoryTest, TestServiceFactoryBundleScopeErrorConditions)
{
  ...
  ASSERT_EQ(
    "1", serviceRefs[0].GetProperty(std::string("returns_nullptr")).ToString());
