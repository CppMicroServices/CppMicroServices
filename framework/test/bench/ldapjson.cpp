...
static void
LDAPNested(benchmark::State& state)
{
    ...
    // case 1: index of array not known
    {
        ...
        for (auto _ : state)
        {
            ...
            for (auto const& anAny : vec)
            {
                Any aaa = ref_any_cast<AnyMap const>(anAny).AtCompoundKey("properties.mw.startup_plugins", Any());
                if (!aaa.Empty())
                {
                    res = res || filt.Match(ref_any_cast<AnyMap const>(aaa));
                }
            }
        }
    }
    ...
    // case 2: index of array known
    {
        ...
        for (auto _ : state)
        {
            ...
            if (!aaa.Empty())
            {
                res = filt.Match(ref_any_cast<AnyMap const>(aaa));
            }
        }
    }
...
static void
JMESPathNested(benchmark::State& state)
{
    ...
    // "scr.components[?properties.mw.startup_plugins.priority=='required'].properties.mw.startup_plugins.priority";
...
static void
JsonPathNested(benchmark::State& state)
{
    std::string path(US_FRAMEWORK_SOURCE_DIR);
    path += "/test/bundles/dummyService/resources/manifest.json";

    std::vector<std::string> filter_strs { "$.bundle_configuration[?(@.mode=='cloud')]",
                                           "$.a.b.c[?(@.e=='found')].e",
                                           "$..[?(@.e=='found')]",
                                           "$..[?(@.bundle_start=='greedy')]",
                                           "$.scr.components[?(@.properties.mw.startup_plugins.priority=='required')]."
                                           "properties.mw.startup_plugins.priority",
                                           "$.scr.components.0.properties.mw.startup_plugins.priority",
                                           "$.a.b.c[?(@.e=='found')].e" };
    ...
    for (auto _ : state)
    {
        for (auto const& filter_str : filter_strs)
        {
...
static void
AnyMapToJsoncons(benchmark::State& state) // used in proporties.cpp to convert anymap to json_props
{
    auto framework = FrameworkFactory().NewFramework();
    framework.Start();
    auto context = framework.GetBundleContext();
...
#if NEVER
static void
JsonconsToAnyMap(benchmark::State& state)
{
...
#if NEVER
static void
ParseManifestToString(benchmark::State& state) // ask
{
    std::string path(US_FRAMEWORK_SOURCE_DIR);
    path += "/test/bundles/dummyService/resources/manifest.json";
...
static void
ParseManifestToAnyMap(benchmark::State& state)
{ // ask
    std::string path(US_FRAMEWORK_SOURCE_DIR);
    path += "/test/bundles/dummyService/resources/manifest.json";
...
static void
ParseManifestToJsoncons(benchmark::State& state) // used in proporties.cpp
{
    std::string path(US_FRAMEWORK_SOURCE_DIR);
    path += "/test/bundles/dummyService/resources/manifest.json";
    for (auto _ : state)
...
static void
ParseManifestToJsonconsFromStringStream(benchmark::State& state)
{
    std::string path(US_FRAMEWORK_SOURCE_DIR);
    path += "/test/bundles/dummyService/resources/manifest.json";
    std::ifstream t(path);
...
static void
ParseManifestToAnyMapFromStringStream(benchmark::State& state)
{
    std::string path(US_FRAMEWORK_SOURCE_DIR);
    path += "/test/bundles/dummyService/resources/manifest.json";
    std::ifstream t(path);
...
static void
ParseManifestToBoth(benchmark::State& state)
{
    std::string path(US_FRAMEWORK_SOURCE_DIR);
    path += "/test/bundles/dummyService/resources/manifest.json";
    BundleManifest bm;
...
static void
ParseManifestToJsonconsThenToAnyMap(benchmark::State& state)
{
    std::string path(US_FRAMEWORK_SOURCE_DIR);
    path += "/test/bundles/dummyService/resources/manifest.json";
    BundleManifest bm;
...
static void
ParseManifestToAnyMapThenToJsoncons(benchmark::State& state)
{
    std::string path(US_FRAMEWORK_SOURCE_DIR);
    path += "/test/bundles/dummyService/resources/manifest.json";
    BundleManifest bm;
...
static void
LDAPNestedServiceReference(benchmark::State& state)
{
    ...
    auto services
        = scopedFramework.framework.GetBundleContext().GetServiceReference("cppmicroservices::TestBundleLQService");
