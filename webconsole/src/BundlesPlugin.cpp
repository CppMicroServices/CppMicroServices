...
bool SetIfExists(AbstractWebConsolePlugin::TemplateData& entry, std::string const& field, std::string const& key, AnyMap const& headers)
{
  try
  {
...
std::string SizeTag(const std::pair<std::size_t, std::size_t>& p)
{
  ...
  if (p.first > 1000)
  {
    if (p.first > 1000 * 1000)
    {
      ...
      {
        ss << std::lround((static_cast<double>(p.first) / 1000.0 / 1000.0 / 1000.0));
  ...
  ss << " (" << std::lround((static_cast<double>(p.second) / factor)) << ") " << postfix;
  return ss.str();
}
...
void BundlesPlugin::RenderContent(HttpServletRequest& request, HttpServletResponse& response)
{
  switch (static_cast<RequestType>(any_cast<int>(request.GetAttribute(REQ_BUNDLE_TYPE))))
  {
  case RequestType::MainPage:
  {
    BundleResource res = GetBundleContext().GetBundle().GetResource("/templates/bundles.html");
    if (!res)
      break;
  ...
  {
    BundleResource res = GetBundleContext().GetBundle().GetResource("/templates/bundle.html");
    if (!res)
      break;
  ...
  case RequestType::Resource:
  {
    ...
    std::string resPath = any_cast<std::string>(request.GetAttribute(REQ_BUNDLE_RES_PATH));
  ...
  else if (pathInfo.size() > 9 && pathInfo.compare(0, 9, "/bundles/") == 0)
  {
    try
    {
      ...
      else
      {
        ...
        if (sub.size() > res_prefix.size() && sub.compare(0, res_prefix.size(), res_prefix) == 0)
        {
...
AbstractWebConsolePlugin::TemplateData BundlesPlugin::GetBundlesData() const
{
  ...
  std::sort(bundles.begin(), bundles.end(), [](Bundle const& b1, Bundle const& b2) {
  ...
  for(auto& bundle : bundles)
  {
    ...
    entry["state"] = static_cast<std::stringstream const&>(std::stringstream() << bundle.GetState()).str();
...
std::pair<std::size_t, std::size_t> BundlesPlugin::GetResourceJsonTree(
    Bundle& bundle, const std::string& parentPath,
    ...
    int level, const std::string& pluginRoot) const
{
  ...
  if (currResource.IsFile())
  {
    ...
    json +=
        indent + "{ text : '" + currResource.GetName() + "',\n" +
        indent + "  icon : 'glyphicon glyphicon-open',\n" +
        indent + "  href : '" + pluginRoot + "/" + NumToString(bundle.GetBundleId()) + "/resources" + currResource.GetResourcePath() + "',\n" +
        indent + "  tags : ['Size: " + SizeTag(std::make_pair(currResource.GetSize(), currResource.GetCompressedSize())) + "',\n" +
  ...
  {
    json += indent + "{ text : '" + currResource.GetResourcePath().substr(parentPath.size()) + "',\n";
    ...
    else
    {
      ...
      {
        BundleResource childResource = bundle.GetResource(currResource.GetResourcePath() + "/" + child);
      ...
      {
        auto size = GetResourceJsonTree(bundle, currResource.GetResourcePath(), childDir, json, level+1, pluginRoot);
      ...
      {
        auto size = GetResourceJsonTree(bundle, currResource.GetResourcePath(), *iter, json, level+1, pluginRoot);
        totalSize.first += size.first;
        totalSize.second += size.second;
        json += indent + ",\n";
      }
...
void BundlesPlugin::GetBundleData(long id, TemplateData& data, const std::string& pluginRoot) const
{
  ...
  for (auto& s : bundle.GetRegisteredServices())
  {
    ...
    service["ranking"] = s.GetProperty(Constants::SERVICE_RANKING).ToStringNoExcept();
    service["scope"] = s.GetProperty(Constants::SERVICE_SCOPE).ToStringNoExcept();
