        ...
        , public ServiceTrackerCustomizer<void>
    {
      public:
        ...
        Start(BundleContext context)
        {
            ...
            cppmicroservices::LDAPFilter filter(cppmicroservices::LDAPProp(Constants::OBJECTCLASS)
        ...
        void
        RegisterServices(std::shared_future<void> f, int c)
        {
            f.wait();
        ...
        AddingService(ServiceReferenceU const& reference)
        {
            if (reference.GetProperty("i") == 5)
            {
                ...
                if (!additionalReg.v)
                {
                    ...
                    additionalReg.v = context.RegisterService(std::make_shared<InterfaceMap const>(im));
                }
        ...
        void
        RemovedService(ServiceReferenceU const& /*reference*/, InterfaceMapConstPtr const& /*service*/)
        {
            auto l = additionalReg.Lock();
            if (additionalReg.v)
