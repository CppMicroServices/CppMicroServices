...
void testDefaultAutoLoadPath(bool autoLoadEnabled, Framework* framework)
{
  ...
  Any loadedBundles = bundleAL->GetProperty(Bundle::PROP_AUTOLOADED_BUNDLES());
  ...
  if (autoLoadEnabled)
  {
    US_TEST_CONDITION_REQUIRED(bundleAL_1 != NULL, "Test for existing auto-loaded bundle TestBundleAL_1")
    US_TEST_CONDITION(bundleAL_1->GetName() == "TestBundleAL_1", "Test bundle name")
    US_TEST_CONDITION_REQUIRED(!loadedBundles.Empty(), "Test for PROP_AUTOLOADED_BUNDLES property")
    US_TEST_CONDITION_REQUIRED(loadedBundles.Type() == typeid(std::vector<std::string>), "Test for PROP_AUTOLOADED_BUNDLES property type")
    std::vector<std::string> loadedBundlesVec = any_cast<std::vector<std::string> >(loadedBundles);
    US_TEST_CONDITION_REQUIRED(loadedBundlesVec.size() == 1, "Test for PROP_AUTOLOADED_BUNDLES vector size")
    US_TEST_CONDITION_REQUIRED(loadedBundlesVec[0] == bundleAL_1->GetName(), "Test for PROP_AUTOLOADED_BUNDLES vector content")
  ...
  {
    US_TEST_CONDITION_REQUIRED(bundleAL_1 == NULL, "Test for non-existing auto-loaded bundle TestBundleAL_1")
    US_TEST_CONDITION_REQUIRED(loadedBundles.Empty(), "Test for empty PROP_AUTOLOADED_BUNDLES property")
  }
...
#ifdef US_ENABLE_AUTOLOADING_SUPPORT
  US_TEST_CONDITION_REQUIRED(bundleAL2_1 != NULL, "Test for existing auto-loaded bundle TestBundleAL2_1")
...
#else
  US_TEST_CONDITION_REQUIRED(bundleAL2_1 == NULL, "Test for non-existing aut-loaded bundle TestBundleAL2_1")
#endif
