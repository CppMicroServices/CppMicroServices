...
CMAsyncWorkService::~CMAsyncWorkService() noexcept
{
  asyncWorkService.reset();
}

void CMAsyncWorkService::StopTracking()
{
  if (serviceTracker) {
...
CMAsyncWorkService::AddingService(
...
void CMAsyncWorkService::ModifiedService(
...
void CMAsyncWorkService::RemovedService(
  const ServiceReference<
    cppmicroservices::async::AsyncWorkService>& /* reference */,
...
void CMAsyncWorkService::post(std::packaged_task<void()>&& task)
{
  auto currAsync = std::atomic_load(&asyncWorkService);
