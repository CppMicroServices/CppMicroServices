...
template<class T> class remove_reference_if_valid
{
  ...
  typedef typename cppmsboost::remove_reference<T>::type plainT;
public:
  typedef typename IF<
    cppmsboost::is_function<plainT>::value,
    T,
    plainT
  >::RET type;
...
template<class T> struct remove_reference_and_cv {
   typedef typename cppmsboost::remove_cv<
     typename cppmsboost::remove_reference<T>::type
   >::type type;
};
...
template<int N, class T> struct tuple_element_as_reference {   
  typedef typename
     cppmsboost::tuples::access_traits<
       typename cppmsboost::tuples::element<N, T>::type
     >::non_const_type type;
};
...
struct parameter_traits_<cppmsboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const cppmsboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<volatile cppmsboost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const volatile cppmsboost::reference_wrapper<T>, Any >{
  typedef T& type;
};
...
struct const_copy_argument {
  typedef typename 
    detail::parameter_traits_<
      T,
      typename detail::IF<cppmsboost::is_function<T>::value, T&, const T>::RET
    >::type type;
};
...
class bind_type_generator {
  ...
  BOOST_STATIC_CONSTANT(int, nof_elems = cppmsboost::tuples::length<args_t>::value);
