...
namespace cppmicroservices
{
    ...
    namespace testing
    {
        ...
        HighPrecisionTimer::HighPrecisionTimer() : startTime(0)
        {
            if (timeConvert == 0)
            {
        ...
        long long
        HighPrecisionTimer::ElapsedMilli()
        {
        ...
        long long
        HighPrecisionTimer::ElapsedMicro()
        {
            uint64_t current = mach_absolute_time();
        ...
        Bundle
        InstallLib(BundleContext frameworkCtx,
                   ...
                   cppmicroservices::AnyMap const& bundleManifest)
        {
            ...
            bundles = frameworkCtx.InstallBundles(LIB_PATH + util::DIR_SEP + US_LIB_PREFIX + libName + US_LIB_POSTFIX
                                                      + US_LIB_EXT,
                                                  bundleManifest);
#else
        ...
        void
        ChangeDirectory(std::string const& destdir)
        {
            errno = 0;
        ...
        static char const validLetters[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";

        // A cross-platform version of the mkstemps function
        static int
        mkstemps_compat(char* tmpl, int suffixlen)
        {
...
/* The number of times to attempt to generate a temporary file.  To
   conform to POSIX, this must be no smaller than TMP_MAX.  */
            ...
            unsigned int const attempts = TMP_MAX;
#else
            unsigned int const attempts = ATTEMPTS_MIN;
#endif
...
/* Get some more or less random data.  */
        ...
        char*
        mkdtemps_compat(char* tmpl, int suffixlen)
        {
            static unsigned long long value = 0;
            int savedErrno = errno;
...
/* Get some more or less random data.  */
        ...
        File::File() : FileDescr(-1), Path() {}

        File::File(int fd, std::string const& path) : FileDescr(fd), Path(path) {}

        File::File(File&& o) : FileDescr(o.FileDescr), Path(std::move(o.Path)) { o.FileDescr = -1; }
        ...
        TempDir::TempDir(std::string const& path) : Path(path) {}

        TempDir::TempDir(TempDir&& o) : Path(std::move(o.Path)) {}
        ...
        File
        MakeUniqueTempFile(std::string const& base)
        {
            auto const tmpStr = base + util::DIR_SEP + "usfile-XXXXXX";
            std::vector<char> tmpChars(tmpStr.c_str(), tmpStr.c_str() + tmpStr.length() + 1);
        ...
        Bundle
        GetBundle(std::string const& bsn, BundleContext context)
        {
            if (!context)
