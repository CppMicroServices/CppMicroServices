...
namespace cppmicroservices
{
    ...
    {
        CoreBundleContextHolder(std::unique_ptr<CoreBundleContext> ctx) : ctx(std::move(ctx)) {}

        ~CoreBundleContextHolder()
        {
            auto const state = ctx->systemBundle->GetState();
    ...
    Framework
    FrameworkFactory::NewFramework(FrameworkConfiguration const& configuration, std::ostream* logger)
    {
    ...
    Framework
    FrameworkFactory::NewFramework(std::map<std::string, Any> const& configuration, std::ostream* logger)
    {
