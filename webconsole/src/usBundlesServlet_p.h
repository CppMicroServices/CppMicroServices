...
{
  BundlesHtml(us::BundleContext* context)
...
private:
  ...
  virtual bool sendHtml(struct mg_connection *conn, const std::string& uri)
  {
    ...
    us::BundleResource resource = m_Context->GetBundle()->GetResource(resourceUri);
    if (!resource)
    {
    ...
    virtual bool sendJson(struct mg_connection *conn, const std::string& uri)
    {
      ...
      {
        json += "{ \"bundles\": [";
        std::vector<us::Bundle*> bundles = m_Context->GetBundles();
        for(std::vector<us::Bundle*>::iterator iter = bundles.begin(), endIter = bundles.end();
            iter != endIter; ++iter)
        {
          ...
          us::Any description = (*iter)->GetProperty(us::Bundle::PROP_DESCRIPTION());
          json += "  \"description\": \"" + (description.Empty() ? std::string() : description.ToString()) + "\",\n";
          json += "  \"version\": \"" + (*iter)->GetVersion().ToString() + "\",\n";
          us::Any vendor = (*iter)->GetProperty(us::Bundle::PROP_VENDOR());
  ...
  virtual bool sendJson(struct mg_connection* conn, const std::string& uri)
  {
    ...
    {
      us::Any value = bundle->GetProperty(*iter);
      if (!value.Empty())
      {
    ...
    us::BundleResource resource = bundle->GetResource("/");
    if (resource.IsValid())
    {
      json += ",\n\"resources\":\n";
    ...
    std::vector<us::ServiceReferenceU> registeredServices = bundle->GetRegisteredServices();
  ...
  void GetJsonRepresentation(us::Bundle* bundle, const us::BundleResource& currResource, std::string& json, int level)
  {
    std::string indent;
    ...
    else
    {
      ...
      else
      {
        ...
        std::vector<us::BundleResource> childDirs;
        std::vector<us::BundleResource> childFiles;
        for (std::vector<std::string>::iterator iter = children.begin(),
             endIter = children.end(); iter != endIter; ++iter)
        {
          us::BundleResource childResource = bundle->GetResource(currResource.GetResourcePath() + "/" + *iter);
        ...
        for (std::vector<us::BundleResource>::iterator iter = childDirs.begin(),
        ...
        for (std::vector<us::BundleResource>::iterator iter = childFiles.begin(),
