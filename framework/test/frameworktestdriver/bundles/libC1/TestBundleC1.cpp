...
class TestBundleC1Activator : public BundleActivator, public ServiceTrackerCustomizer<void>
{
public:
  ...
  void Start(BundleContext context)
  {
    ...
    cppmicroservices::LDAPFilter filter(cppmicroservices::LDAPProp(Constants::OBJECTCLASS) == "org.cppmicroservices.c1.*" && cppmicroservices::LDAPProp("i") == 5);
    ...
    for (int c = 0; c < 10; ++c)
    {
      threads.emplace_back(&TestBundleC1Activator::RegisterServices, this, f, c);
    }
  ...
  void Stop(BundleContext)
  {
    ...
    if (count != 0) throw std::logic_error("Not unregistered all services");
  }
  ...
  void RegisterServices(std::shared_future<void> f, int c)
  {
    ...
    for (int i = 0; i < 10; ++i)
    {
      ...
      InterfaceMap im;
      im[std::string("org.cppmicroservices.c1.") + testing::ToString(c)] = std::make_shared<int>(1);
