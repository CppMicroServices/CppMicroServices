...
class ModulePEFile : public ModuleObjFile, private PEType
{
public:

  typedef typename PEType::Ohdr Ohdr;

  ModulePEFile(std::ifstream& fs, const COFFHeader& coffHeader)
    ...
    , m_SectionHeaders(NULL)
  {
    ...
    // (we rely on the _us_import_module_initializer_<module-name> symbol)
    ...
    fs.read(reinterpret_cast<char*>(m_SectionHeaders), sizeof(SectionHeader) * coffHeader.NumberOfSections);
    ...
    // get the "signature" _us_import_module_initializer_<module-name>
    uint32_t* nameRVAs = new uint32_t[exportDir.NumberOfNames];
    fs.seekg(GetOffset(exportDir.AddressOfNames));
    fs.read(reinterpret_cast<char*>(nameRVAs), sizeof *nameRVAs * exportDir.NumberOfNames);
    for (uint32_t i = 0; i < exportDir.NumberOfNames; ++i)
    {
      fs.seekg(GetOffset(nameRVAs[i]));
      std::string name;
      std::getline(fs, name, '\0');
      if (this->ExtractModuleName(name, m_ModuleName))
      {
        break;
      }
    }
...
ModuleObjFile* CreateModulePEFile(const char* /*selfName*/, const std::string& fileName)
{
  struct stat peStat;
  ...
  {
    return new ModulePEFile<PE<OPTIONAL_PE32_MAGIC> >(peFile, coffHeader);
  }
  else if (stateMagic == OPTIONAL_PE64_MAGIC)
  {
    return new ModulePEFile<PE<OPTIONAL_PE64_MAGIC> >(peFile, coffHeader);
  }
  else
  {
