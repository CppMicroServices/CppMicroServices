...
namespace cppmicroservices
{
  ...
  {"m" , AnyMap { ORDERED_MAP, {
    ...
    TEST(AnyMapTest, AnyMapIteratorTypeChecked)
    {
        std::string key = "hello";
        std::string val = "world";
        cppmicroservices::AnyMap uociMap {
            cppmicroservices::AnyMap::UNORDERED_MAP_CASEINSENSITIVE_KEYS, {
                { key, val }
            }
        };
        cppmicroservices::AnyMap uociMapMoved(uociMap);
        for (auto it = uociMapMoved.beginUOCI_TypeChecked(); it != uociMapMoved.endUOCI_TypeChecked(); it++)
        {
            ASSERT_STREQ(key.c_str(), it->first.c_str());
            ASSERT_STREQ(val.c_str(), any_cast<std::string>(it->second).c_str());
        }

        std::ostringstream os;
        any_value_to_cpp(os, uociMapMoved);
        ASSERT_EQ(os.str(), R"(AnyMap { UNORDERED_MAP_CASEINSENSITIVE_KEYS, {{"hello" , std::string("world")}}})");

        os.str("");
        uociMapMoved.erase(key);
        any_value_to_cpp(os, uociMapMoved);
        ASSERT_EQ(os.str(), R"(AnyMap { UNORDERED_MAP_CASEINSENSITIVE_KEYS, {}})");
    }
