...
void TestManagedServiceFactoryImpl2::Activate(
...
void TestManagedServiceFactoryImpl2::Removed(std::string const& pid)
{
  std::lock_guard<std::mutex> lk(m_removedMtx);
  ++m_removedCallCount[pid];
}

int TestManagedServiceFactoryImpl2::getUpdatedCounter(std::string const& pid)
{
  std::lock_guard<std::mutex> lk(m_updatedMtx);
  return m_updatedCallCount[pid];
}

int TestManagedServiceFactoryImpl2::getRemovedCounter(std::string const& pid)
{
  std::lock_guard<std::mutex> lk(m_removedMtx);
...
TestManagedServiceFactoryImpl2::create(std::string const& config)
{
  ...
  std::lock_guard<std::mutex> lk(m_updatedMtx);
  try {
    return std::make_shared<TestManagedServiceFactoryServiceImpl2>(
