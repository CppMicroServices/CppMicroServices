...
TEST_F(BundleTrackerConcurrencyTest, ConcurrentOpenCloseWorks)
{
  ...
  for (size_t i = 0; i <= numThreads; ++i) {
    futures.push_back(
      std::async(std::launch::async, [i, &bundleTracker, &gateFuture]() {
        gateFuture.get();
...
TEST_F(BundleTrackerConcurrencyTest, NoRaceConditionForRemovingChangingBundle)
{
  auto customizer = std::make_shared<MockCustomizer2>();
  auto bundleTracker =
    std::make_unique<BundleTracker<>>(context, all_states, customizer);
  ...
  EXPECT_CALL(*customizer, AddingBundle)
    ...
    .WillOnce(::testing::DoAll(::testing::Invoke(increment),
                               ::testing::ReturnArg<0>()));
  EXPECT_CALL(*customizer, ModifiedBundle)
    .Times(::testing::AtLeast(2))
    .WillRepeatedly(::testing::Invoke(increment));
