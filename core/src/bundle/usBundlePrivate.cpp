  ...
  , lib(info->location)
{
  ...
  {
    const std::vector<std::string> loadedBundleNames = AutoLoadBundles(this->info, this->coreCtx);
    if (!loadedBundleNames.empty())
    {
      bundleManifest.SetValue(Bundle::PROP_AUTOLOADED_BUNDLES(), Any(loadedBundleNames));
    }
  }
#endif
}
