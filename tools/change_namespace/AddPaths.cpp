...
void ChangeNamespaceImpl::add_path(const fs::path& p)
{
...
void ChangeNamespaceImpl::add_directory(const fs::path& p)
{
   // Parse files and directories
   fs::directory_iterator i(m_cppms_path / p);
   fs::directory_iterator j;
   while(i != j)
   {
      std::string s(i->path().string());
      if(m_cppms_path.string().size())
...
void ChangeNamespaceImpl::add_file(const fs::path& p)
{
...
void ChangeNamespaceImpl::add_file_dependencies(const fs::path& p, bool scanfile)
{
    static const std::regex e(R"(^[[:blank:]]*(?://@bcp[[:blank:]]+([^\n]*)\n)?#[[:blank:]]*include[[:blank:]]*["<]([^">]+)[">])");
    ...
    while (i != j)
    {
        ...
        try {
            std::string discard_message = *i;
            ++i;
            if (discard_message.size())
            {
            ...
            if (fs::exists(test_file) && !fs::is_directory(test_file) && (p.parent_path().string() != "CppMicroServices"))
            {
