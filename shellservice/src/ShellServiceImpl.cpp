...
//};
//
//void ShellServiceImpl::Impl::InitSymbols()
//{
...
std::string triml(const std::string &str) {
    ...
    if (strBegin != std::string::npos) {
        returnString = str.substr(strBegin);
    }
    return returnString;
}
    
std::string trimString(const std::string &cmd) {
    
    // remove leading whitespace
    const auto strBegin = cmd.find_first_not_of(' ');
    if (strBegin == std::string::npos) {
        // no command to process
        std::cout << "Please run a valid command" << std::endl;
        return "";
    }
    
    const auto strEnd = cmd.find_last_not_of(' ');
    const auto strRange = strEnd - strBegin + 1;
    
    return cmd.substr(strBegin, strRange);
}
    
bool validateNumberOfQuotes(const std::string &cmd) {
    size_t n = std::count(cmd.begin(), cmd.end(), '\"');
    return (n%2 == 0);
}
    
std::vector<std::string> parseCommandString(const std::string &cmd) {
    
    std::string command = trimString(cmd);
    auto commandStringLength = command.size();
    
    if (!validateNumberOfQuotes(command)) {
        std::cout << "Please make sure all your double-quotes are closed." << std::endl;
    }
    ...
    while (commandStringLength > 0) {
        if (command.substr(0,1) == "\"") {
            pos = command.substr(1,command.size()-1).find_first_of("\"") + 1;
            parsedArguments.push_back(command.substr(1,pos-1));
            if (pos < commandStringLength-1) {
                command = triml(command.substr(pos+1));
                commandStringLength = command.size();
            } else {
                break;
            }
        } else {
            ...
            if (pos == std::string::npos) {
                parsedArguments.push_back(command);
                break;
            } else {
                parsedArguments.push_back(command.substr(0,pos));
...
void ShellServiceImpl::ExecuteCommand(const std::string& cmd)
{
    auto commandArguments = parseCommandString(cmd);
    ...
    cppmicroservices::BundleContext bc = GetBundleContext();
    
    std::string filter = "(commandName=" + command + ")";
    std::vector<cppmicroservices::ServiceReference<cppmicroservices::ShellCommandInterface>> srList = bc.GetServiceReferences<cppmicroservices::ShellCommandInterface>(filter);
    
    if (srList.size()) {
        std::shared_ptr<cppmicroservices::ShellCommandInterface> service = bc.GetService(srList.at(0));
        try {
            service->doCommand(bc, commandArguments);
        } catch (const std::exception& e) {
...
//std::vector<std::string> ShellServiceImpl::GetCompletions(const std::string& in)
//{
...
//}
//
//void ShellServiceImpl::LoadSchemeResource(const BundleResource& res)
//{
//  std::cout << "Reading " << res.GetResourcePath() << std::endl;
