...
public:
    ...
    void SetUp() override
    {
      ...
      #else
        bundle =
            cppmicroservices::testing::GetBundle("TestBundleLQ", f.GetBundleContext());
      #endif
    }
...
TEST_F(LDAPQueryTest, TestLDAPFilterMatchBundle)
{
    ...
    ASSERT_TRUE(ldapMatchCase.Match(bundle));

    // Testing case-insensitive key (should still pass)
    ASSERT_TRUE(ldapKeyMismatchCase.Match(bundle));

    // Testing case-insensitive value (should fail)
    ASSERT_FALSE(ldapValueMismatchCase.Match(bundle));
}
...
TEST_F(LDAPQueryTest, TestLDAPFilterMatchNoException)
{
    ...
    // Testing no exception is thrown.
    ASSERT_NO_THROW(ldapMatch.Match(bundle));

    // Testing key match
    ASSERT_TRUE(ldapMatch.Match(bundle));
