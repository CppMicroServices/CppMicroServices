...
class ModuleElfFile : public ModuleObjFile, private ElfType
{
public:
  ...
  ModuleElfFile(std::ifstream& fs, std::size_t fileSize)
    : m_SectionHeaders(NULL)
  {
    ...
    for (std::size_t symIndex = 0; symIndex < sectionSize / dynsymHdr->sh_entsize; ++symIndex, ++symEntry)
    {
      ...
      if (this->ExtractModuleName(symName, m_ModuleName))
      {
        break;
      }
    }
...
ModuleObjFile* CreateModuleElfFile(const char* selfName, const std::string& fileName)
{
  struct stat elfStat;
  ...
  {
    return new ModuleElfFile<Elf<ELFCLASS32> >(elfFile, fileSize);
  }
  else if (elfIdent[EI_CLASS] == ELFCLASS64)
  {
    return new ModuleElfFile<Elf<ELFCLASS64> >(elfFile, fileSize);
  }
  else
  {
