...
TEST_F(BundleTrackerMethodTest, GetTrackedWorks)
{
  ...
  EXPECT_EQ(0, tracked.size())
    << "GetTracked() should return an empty map before Open()";
  ...
  tracked = bundleTracker.GetTracked();
  EXPECT_EQ(0, tracked.size())
    << "GetTracked() should return an empty map after Close()";
}
...
TEST_F(BundleTrackerMethodTest, IsEmptyWorks)
{
  ...
  Bundle bundle = cppmicroservices::testing::InstallLib(
    framework.GetBundleContext(), "TestBundleA");
  ASSERT_EQ(bundle, bundleTracker.GetObject(bundle))
    << "BundleTracker failed to track a bundle";
  EXPECT_FALSE(bundleTracker.IsEmpty())
    << "BundleTracker should not be empty when a bundle is tracked";
...
TEST_F(BundleTrackerMethodTest, RemoveWorks)
{
  ...
  bool bundleStillTracked =
    std::find(bundlesTracked.begin(), bundlesTracked.end(), bundle) !=
    bundlesTracked.end();
  EXPECT_FALSE(bundleStillTracked)
...
TEST_F(BundleTrackerMethodTest, ReopeningTrackerWorks)
{
  ...
  EXPECT_EQ(2, bundleTracker.Size())
    << "BundleTracker is tracking bundles incorrectly after being reopened";
...
{
  auto stateMask = BundleTracker<>::CreateStateMask(Bundle::State::STATE_STARTING,
                                    Bundle::State::STATE_STOPPING);
  BundleTracker<> bundleTracker(context, stateMask);
  ASSERT_NO_THROW(bundleTracker.Open()) << "BundleTracker failed to start";
