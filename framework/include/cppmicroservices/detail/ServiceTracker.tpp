...
#    pragma warning(pop)
    ...
    ServiceTracker<S, T>::Open()
    {
        std::shared_ptr<_TrackedService> t;
        {
            ...
            DIAG_LOG(*d->context.GetLogSink()) << "ServiceTracker<S,TTT>::Open: " << d->filter.ToString();
            ...
            try
            {
                ...
                else
                {
                    ...
                    { /* user supplied filter */
                        references = d->GetInitialReferences(
                            std::string(),
    ...
    ServiceTracker<S, T>::Close()
    {
        ...
        std::shared_ptr<_TrackedService> outgoing = d->trackedService.Load();
        {
            ...
            DIAG_LOG(*d->context.GetLogSink()) << "ServiceTracker<S,TTT>::close:" << d->filter.ToString();
            outgoing->Close();
        ...
        if (d->context.GetLogSink()->Enabled())
        {
            if (!d->cachedReference.Load().GetBundle() && d->cachedService.Load() == nullptr)
            {
                DIAG_LOG(*d->context.GetLogSink())
                    << "ServiceTracker<S,TTT>::close[cached cleared]:" << d->filter.ToString();
            }
        }
    }
    ...
    ServiceTracker<S, T>::GetServiceReference() const
    {
        ...
        {
            DIAG_LOG(*d->context.GetLogSink())
                << "ServiceTracker<S,TTT>::getServiceReference[cached]:" << d->filter.ToString();
            return reference;
        }
        DIAG_LOG(*d->context.GetLogSink()) << "ServiceTracker<S,TTT>::getServiceReference:" << d->filter.ToString();
        auto references = GetServiceReferences();
    ...
    ServiceTracker<S, T>::GetService() const
    {
        ...
        if (service)
        {
            DIAG_LOG(*d->context.GetLogSink()) << "ServiceTracker<S,TTT>::getService[cached]:" << d->filter.ToString();
            return service;
        }
        DIAG_LOG(*d->context.GetLogSink()) << "ServiceTracker<S,TTT>::getService:" << d->filter.ToString();
        try
        {
