...
private:
   ...
   */
  class SpellCheckImpl : public US_BASECLASS_NAME, public ISpellCheckService
  {

  private:

    typedef std::map<ServiceReference, IDictionaryService*> RefToServiceType;
  ...
  public:
    ...
    int AddDictionary(const ServiceReference& ref, IDictionaryService* dictionary)
    {
      // Lock the m_refToSvcMap member using your favorite thread library here...
      // MutexLocker lock(&m_refToSvcMapMutex)
    ...
    int RemoveDictionary(const ServiceReference& ref)
    {
      // Lock the m_refToSvcMap member using your favorite thread library here...
      // MutexLocker lock(&m_refToSvcMapMutex)
  ...
  virtual IDictionaryService* AddingService(const ServiceReference& reference)
  {
    IDictionaryService* dictionary = m_context->GetService<IDictionaryService>(reference);
  ...
  virtual void ModifiedService(const ServiceReference& /*reference*/, IDictionaryService* /*service*/)
  {
    // do nothing
  }

  virtual void RemovedService(const ServiceReference& reference, IDictionaryService* /*service*/)
  {
  ...
  ServiceRegistration m_spellCheckReg;
