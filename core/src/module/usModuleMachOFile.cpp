...
class ModuleMachOFile : public ModuleObjFile, private MachOType
{
public:
  ...
  ModuleMachOFile(std::ifstream& fs, std::size_t fileOffset)
  {
    fs.seekg(fileOffset);
    Mhdr mhdr;
      ...
      else if (lcmd.cmd == LC_DYSYMTAB)
      {
        ...
        if (dysymtab.nextdefsym != 0)
        {
          ...
          for (std::size_t i = 0; i < dysymtab.nextdefsym; ++i, ++extSym)
          {
            ...
            if (this->ExtractModuleName(&strtab[extSym->n_un.n_strx + 1], m_ModuleName))
            {
              break;
            }
          }
...
// running module (the host system) directly from the file
...
ModuleObjFile* CreateModuleMachOFile(const char* /*selfName*/, const std::string& fileName)
{
  ...
  if (matchingIdent[0] == MH_MAGIC)
  {
    ...
    return new ModuleMachOFile<MachO<MH_MAGIC> >(machFile, matchingIdent[2]);
  }
  else if (matchingIdent[0] == MH_MAGIC_64)
  {
    //std::cout << "Mach-O (64-bit) binary" << std::endl;
    return new ModuleMachOFile<MachO<MH_MAGIC_64> >(machFile, matchingIdent[2]);
  }
  else
  {
