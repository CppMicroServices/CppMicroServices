...
namespace cppmicroservices
{
    ...
    namespace detail
    {
        ...
        Any const& AtCompoundKey(std::vector<Any> const& v, absl::string_view const& key);

        Any const&
        AtCompoundKey(AnyMap const& m, absl::string_view const& key)
        {
            auto pos = key.find(".");
        ...
        Any const&
        AtCompoundKey(std::vector<Any> const& v, absl::string_view const& key)
        {
            auto pos = key.find(".");
            if (pos != AnyMap::key_type::npos)
            {
        ...
        Any AtCompoundKey(std::vector<Any> const& v, absl::string_view const& key, Any&& defaultVal);

        Any
        AtCompoundKey(AnyMap const& m, absl::string_view const& key, Any&& defaultVal)
        {
            auto pos = key.find(".");
            if (pos != AnyMap::key_type::npos)
            {
        ...
        Any
        AtCompoundKey(std::vector<Any> const& v, absl::string_view const& key, Any&& defaultval)
        {
            auto pos = key.find(".");
            auto const head = key.substr(0, pos);
