...
private:
  ...
  US_HASH_FUNCTION_FRIEND(BundleResource);
 ...
 * \ingroup MicroServices
 */
US_Core_EXPORT std::ostream& operator<<(std::ostream& os, const US_PREPEND_NAMESPACE(BundleResource)& resource);

US_HASH_FUNCTION_NAMESPACE_BEGIN
US_HASH_FUNCTION_BEGIN(US_PREPEND_NAMESPACE(BundleResource))
  return arg.Hash();
